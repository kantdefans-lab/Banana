...
const WAVESPEED_MODELS_API = '/api/wavespeed/models?media=image';
...
function extractImageUrls(result: any): string[] {
  ...
  // 🎯 最小化修复：添加对KIE.AI格式的支持
  // 根据你的日志，URL在 rawData.response.resultUrls
  if (result.rawData?.response?.resultUrls && Array.isArray(result.rawData.response.resultUrls)) {
    return result.rawData.response.resultUrls.filter((url: string) => 
      url && typeof url === 'string'
    );
  }

  // 🎯 添加对 resultUrls 字段的支持
  if (result.resultUrls && Array.isArray(result.resultUrls)) {
    return result.resultUrls.filter((url: string) => 
      url && typeof url === 'string'
    );
  }

  // 🎯 添加对 result_urls 字段的支持
  if (result.result_urls && Array.isArray(result.result_urls)) {
    return result.result_urls.filter((url: string) => 
      url && typeof url === 'string'
    );
  }
  ...
  if (result.imageUrls && Array.isArray(result.imageUrls)) {
    return result.imageUrls.filter((url: string) => 
      url && typeof url === 'string'
    );
  }
...
export function ImageGenerator({
  ...
  const modelsForTab = useMemo(() => {
    const tabType = activeTab === 'text-to-image' ? 'text-to-image' : 'image-to-image';
    return (wavespeedModels || []).filter((m) => String(m.type || '') === tabType);
  ...
  const handleTabChange = (value: string) => {
    ...
    const nextModels = (wavespeedModels || []).filter((m) =>
      String(m.type || '') === tab
    );
    ...
    const load = async () => {
      ...
      try {
        ...
        if (resp.ok && json?.code === 0 && Array.isArray(json?.data)) {
          const list = json.data as WaveSpeedModelBrief[];
          setWavespeedModels(list);
        } else {
          setWavespeedModels([]);
      ...
      try {
        ...
        const imageUrls = Array.from(
          new Set<string>([
            ...
            ...(Array.isArray((data as any)?.imageUrls) ? (data as any).imageUrls : []),
    ...
    try {
      ...
      const resp = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          ...
          ...(isTextToImageMode ? {} : { filesUrl: referenceImageUrls }),
          options,
        }),
      });
  ...
  const handleDownloadImage = async (image: GeneratedImage) => {
    ...
    try {
      ...
      console.log('📥 开始下载图片:', image.url);
      
      // 方法1：直接创建链接并点击（最简单直接）
      const link = document.createElement('a');
      link.href = image.url;
      link.download = `ai-generated-${Date.now()}-${image.id}.png`;
      ...
      // 显示消息
      toast.success('图片下载已开始，请检查浏览器下载列表');
      ...
      // 备用方案：直接打开新窗口
      try {
        const newWindow = window.open(image.url, '_blank', 'noopener,noreferrer');
        if (newWindow) {
          toast.success('已在新窗口打开图片，请右键保存');
        } else {
          // 如果弹出窗口被阻止，提示用户
          toast.error('下载失败，请右键图片选择"图片另存为"');
        }
      } catch (windowError) {
        toast.error('下载失败，请复制链接手动下载');
      }
    } finally {
