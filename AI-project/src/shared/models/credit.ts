import { and, asc, count, desc, eq, gt, isNull, or, sum, sql } from 'drizzle-orm';
...
export async function consumeCredits({
  ...
  const execute = async (tx: any) => {
    ...
    // 🔥 修复：使用 sql 包装并增加 coalesce 处理 NULL 值
    const [creditsBalance] = await tx
      .select({
        total: sql<string>`coalesce(sum(${credit.remainingCredits}), 0)`,
      })
      .from(credit)
      .where(
        and(
    ...
    // 🔥 增强：增加空值检查逻辑
    const currentTotal = creditsBalance?.total ? parseInt(creditsBalance.total) : 0;
    
    if (currentTotal < credits) {
      throw new Error(
        `Insufficient credits, ${currentTotal} < ${credits}`
      );
    }
