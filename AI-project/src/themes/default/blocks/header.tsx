...
export function Header({ header }: { header: HeaderType }) {
  ...
  const NavMenu = () => {
    ...
    return (
      <NavigationMenu
        ...
        // 修复：将 var(--color-foreground) 改为 var(--foreground)
        className="[--color-muted:color-mix(in_oklch,var(--foreground)_5%,transparent)] [--viewport-outer-px:2rem] **:data-[slot=navigation-menu-viewport]:rounded-none **:data-[slot=navigation-menu-viewport]:border-0 **:data-[slot=navigation-menu-viewport]:bg-transparent **:data-[slot=navigation-menu-viewport]:shadow-none **:data-[slot=navigation-menu-viewport]:ring-0 max-lg:hidden"
      >
  ...
  const MobileMenu = ({ closeMenu }: { closeMenu: () => void }) => {
    return (
      <nav
        role="navigation"
        // 修复：使用 color-mix 替代 --alpha，并将 --color-foreground 改为 --foreground
        className="w-full [--color-border:color-mix(in_oklab,var(--foreground)_5%,transparent)] [--color-muted:color-mix(in_oklab,var(--foreground)_5%,transparent)]"
      >
        <Accordion
          type="single"
          collapsible
                      ...
                      <Link
                        ...
                        className={cn(
                          ...
                          button.variant === 'outline'
                            ...
                            : 'bg-primary text-primary-foreground hover:bg-primary/90 border-[0.5px] border-white/25 shadow-md ring-1 shadow-black/20 [--tw-ring-color:var(--ring-color)] [--ring-color:color-mix(in_oklab,var(--foreground)15%,var(--primary))]'
                        )}
                      >
                        {button.icon && (
